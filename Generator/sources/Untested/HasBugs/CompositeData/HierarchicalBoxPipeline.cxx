...
int main(int argc, char* argv[])
{
  ...
  vtkSmartPointer<vtkActor> shActor = vtkSmartPointer<vtkActor>::New();
  ...
  // // Rendering objects
  // // This one is actually just a vtkPolyData so it doesn't need a
  // hierarchical
  ...
  vtkSmartPointer<vtkActor> ocActor = vtkSmartPointer<vtkActor>::New();
  ocActor->SetMapper(ocMapper);
  ocActor->GetProperty()->SetColor(1, 0, 0);
  ren->AddActor(ocActor);

  // cell 2 point and contour
  vtkSmartPointer<vtkExtractLevel> el = vtkSmartPointer<vtkExtractLevel>::New();
